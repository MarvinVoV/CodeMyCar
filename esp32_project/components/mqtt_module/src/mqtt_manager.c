...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  switch (event->event_id) {
    case MQTT_EVENT_CONNECTED:
      ...
      // 订阅服务端消息
      esp_err_t err = mqtt_subscribe_with_retry(TOPIC_SERVER_CTRL, 0);
      if (err != ESP_OK) {
...
static void handle_command_message(const mqtt_msg_t *msg) {
  // print log
  ESP_LOGI(TAG, "Received MQTT message: topic=%s, payload=%.*s, length=%zu",
  ...
  QueueHandle_t mqtt_send_queue = queue_manager_get_queue(MQTT);
  ...
  if (xQueueSend(mqtt_send_queue, &msg_to_send, pdMS_TO_TICKS(10)) != pdPASS) {
    ESP_LOGE(TAG,
...
esp_err_t mqtt_subscribe_with_retry(const char *topic, int qos) {
  for (int retry = 0; retry < MAX_SUBSCRIBE_RETRIES; retry++) {
    esp_err_t err = mqtt_subscribe(topic, qos);
    if (err == ESP_OK) {
      return ESP_OK;
    }
...
void handle_uart_message_task(void *pvParameters) {
  ...
  while (1) {
    QueueMessage_t uart_send_data = {0};
    BaseType_t receive_status =
        xQueueReceive(uart_send_queue, &uart_send_data, queue_timeout);
    ...
    // validate
    if (uart_send_data.data == NULL || uart_send_data.len == 0) {
      ESP_LOGE(TAG,
               "Invalid data received from uart_send_queue (null data or zero "
               "length)");
      free(uart_send_data.data);
      free(uart_send_data.extra_info);
      continue;
    }

    // TODO  暂时使用该topic
    esp_err_t err = mqtt_publish(TOPIC_DEVICE_SENSOR, uart_send_data.data,
                                 uart_send_data.len, 0);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Failed to publish uart data to MQTT");
    } else {
      // TODO change log level
      ESP_LOGI(TAG, "Uart data published to MQTT successfully");
    }
    free(uart_send_data.data);
    free(uart_send_data.extra_info);
    uart_send_data.data = NULL;
  }
}