...
static const char* TAG = "protocol_receiver";
...
static void try_parse_frame(protocol_receiver* receiver)
{
    ...
    // 移动未处理数据到缓冲区头部(只有处理成功才移动）
    if (unprocessed_start > 0)
    {
        const size_t remaining = receiver->write_pos - unprocessed_start;
...
void protocol_receiver_append(protocol_receiver* receiver, const uint8_t* data, uint16_t len)
{
    ...
    if (receiver->write_pos + len > receiver->buffer_size)
    {
        ...
        {
            // 移动未处理数据到缓冲区头部
            const size_t remaining = receiver->write_pos - processed_len;
        ...
        if (receiver->write_pos + len > receiver->buffer_size)
        {
            ...
            if (!new_buf)
            {
                ...
                if (available_space > 0)
                {
                    ...
                    ESP_LOGE(TAG, "Partial data written (%d bytes), discarded %d bytes", available_space,
                             len- available_space);
                }
                else
                {
                    ESP_LOGE(TAG, " All new data discarded: %d bytes", len);
                }
    ...
    {
        // 处理空间不一致的极端情况（如并发写入）
        ESP_LOGE(TAG, "Buffer space check failed: require=%d, available=%d\n", len, available_space);
